...
class TestStudyCases:
    ...
    def test_chu_the_valid(self):
        for case in self.cases:
            assert case["chu_the"] in ("ca_nhan", "to_chuc"), f"Case {case['id']}: invalid chu_the '{case['chu_the']}'"

    def test_ngay_format(self):
        for case in self.cases:
            assert len(case["ngay_quyet_dinh"]) == 10, f"Case {case['id']}: ngay_quyet_dinh not YYYY-MM-DD"
    ...
    def test_expected_he_thong_fields(self):
        required = {
            ...
            "muc_phat_to_chuc_cu",
            "muc_phat_ca_nhan_cu",
            "muc_phat_to_chuc_moi",
            "muc_phat_ca_nhan_moi",
            "nhan",
            "ghi_chu",
        }
        for case in self.cases:
            ...
            for field in required:
                assert field in exp, f"Case {case['id']}: expected_he_thong missing '{field}'"
    ...
    def test_muc_phat_ranges_valid(self):
        for case in self.cases:
            exp = case["expected_he_thong"]
            for key in ["muc_phat_to_chuc_cu", "muc_phat_ca_nhan_cu", "muc_phat_to_chuc_moi", "muc_phat_ca_nhan_moi"]:
                rng = exp[key]
    ...
    def test_rule_half_in_expected(self):
        for case in self.cases:
            ...
            assert cn_cu[0] == tc_cu[0] // 2, f"Case {case['id']}: ca_nhan_cu min != to_chuc_cu min / 2"
            assert cn_cu[1] == tc_cu[1] // 2, f"Case {case['id']}: ca_nhan_cu max != to_chuc_cu max / 2"
            assert cn_moi[0] == tc_moi[0] // 2, f"Case {case['id']}: ca_nhan_moi min != to_chuc_moi min / 2"
            assert cn_moi[1] == tc_moi[1] // 2, f"Case {case['id']}: ca_nhan_moi max != to_chuc_moi max / 2"
        ...
        for case in self.cases:
            ...
            if case["chu_the"] == "ca_nhan":
                assert cn_cu[0] <= case["muc_phat"] <= cn_cu[1], (
                    f"Case {case['id']}: muc_phat {case['muc_phat']} not in ca_nhan range {cn_cu}"
                )

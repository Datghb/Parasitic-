...
class TestProcessOneWithTichHopNguon:
    ...
    def test_co_bac_bo_chinh_thuc_priority_bump_2(self):
        ...
        provider = _make_provider(
            '{"claim": "tổ chức đăng tin giả bị phạt 20-30 triệu", "keywords": ["tin giả"], "subject": "to_chuc"}'
        )
        ...
        with patch("backend.legal_radar.pipeline.xac_thuc_nguon") as mock_xac_thuc:
            ...
            result = ingestor.process_one(_make_comment("tổ chức đăng tin giả bị phạt 20-30 triệu"))
    ...
    def test_call_to_action_no_source_bump_1(self):
        ...
        kg = _build_kg()
        provider = _make_provider(
            '{"claim": "tẩy chay ngân hàng X ngay đi mọi người", "keywords": ["ngân hàng"], "subject": null}'
        )
        ...
        with patch("backend.legal_radar.pipeline.xac_thuc_nguon") as mock_xac_thuc:
            ...
            ingestor = CommentIngestor(provider, kg)
            result = ingestor.process_one(_make_comment("tẩy chay ngân hàng X ngay đi mọi người"))
    ...
    def test_neutral_claim_no_source_no_bump(self):
        ...
        with patch("backend.legal_radar.pipeline.xac_thuc_nguon") as mock_xac_thuc:
            ...
            ingestor = CommentIngestor(provider, kg)
            result = ingestor.process_one(_make_comment("hôm nay trời đẹp quá"))
    ...
    def test_hieu_lam_base_priority_1(self):
        ...
        kg = _build_kg()
        provider = _make_provider(
            '{"claim": "cá nhân share tin giả bị phạt 20-30 triệu", "keywords": ["tin giả"], "subject": "ca_nhan"}'
        )
        ...
        with patch("backend.legal_radar.pipeline.xac_thuc_nguon") as mock_xac_thuc:
            ...
            ingestor = CommentIngestor(provider, kg)
            result = ingestor.process_one(_make_comment("cá nhân share tin giả bị phạt 20-30 triệu"))
...
class TestValidationChain:
    ...
    def test_sanitize_injection_called_before_llm(self):
        ...
        with patch(
            "backend.legal_radar.pipeline.sanitize_injection",
            wraps=__import__("backend.legal_radar.guardrails", fromlist=["sanitize_injection"]).sanitize_injection,
        ) as mock_sanitize:
            ingestor = CommentIngestor(provider, kg)
    ...
    def test_validate_label_after_engine(self):
        ...
        with (
            patch(
                "backend.legal_radar.pipeline.validate_label",
                wraps=__import__("backend.legal_radar.guardrails", fromlist=["validate_label"]).validate_label,
            ) as mock_validate,
...
class TestIngestCrawledItems:
    def test_post_and_comments_are_appended_once(self, monkeypatch, tmp_path) -> None:
        ...
        provider = _make_provider(
            '{"claim": "tin sáp nhập cần kiểm chứng", "keywords": ["sáp nhập", "đơn vị hành chính"], "subject": null}'
        )

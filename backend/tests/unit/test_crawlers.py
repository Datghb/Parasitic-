...
class TestFacebookCrawler:
    ...
    def test_discover_urls_success(self):
        from legal_radar.crawlers.facebook import _discover_urls
        ...
        with patch.dict(os.environ, {"BRIGHTDATA_API_KEY": "fake"}):
            with patch("legal_radar.crawlers.facebook.requests.post", return_value=mock_post_resp):
                with patch("legal_radar.crawlers.facebook.requests.get", return_value=mock_poll_resp):
    ...
    def test_crawl_one_post_success(self):
        from legal_radar.crawlers.facebook import _crawl_one_post, BD_POSTS_DATASET, BD_COMMENTS_DATASET
        ...
        with patch.dict(os.environ, {"BRIGHTDATA_API_KEY": "fake"}):
            with patch("legal_radar.crawlers.facebook._bd_scrape", side_effect=mock_scrape):
...
class TestCleaner:
    ...
    def test_clean_comment_ui_garbage(self):
        from legal_radar.crawlers.cleaner import clean_comment
...
class TestFilter:
    ...
    def test_is_relevant_sap_nhap_tinh(self):
        from legal_radar.crawlers.filter import is_relevant
    ...
    def test_filter_posts_keeps_relevant(self):
        from legal_radar.crawlers.filter import filter_posts
        posts = [
